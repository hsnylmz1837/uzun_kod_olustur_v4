...
st.caption("Repodaki **data/schema.xlsx** otomatik yüklenir. Gerekirse DEV modda manuel yükleyebilirsiniz.")
...
STATIC_SCHEMA = "false" #os.getenv("STATIC_SCHEMA", "true").lower() == "true"
                ...
                with tabs[i]:
                    ...
                    for _, fld in fields.iterrows():
                        ...
                        if typ in ("select", "multiselect"):
                            ...
                            st.session_state.values[k] = val
                        else:
                            txt = st.text_input(label + (" *" if req else ""), value=str(default) if pd.notna(default) else "", key=f"k_{k}")
                            st.session_state.values[k] = txt

            st.markdown("---")
            c1, c2 = st.columns([1,1])
            with c1:
                if st.button("🔐 Uzun Kodu Oluştur"):
                    code = encode_long_code(s1, s2, row["UrunKodu"], st.session_state.values, fdf)
